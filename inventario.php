    ...
    <table border="1">
        ...
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            echo "<tr>
                ...
                <td>".$row['modelo']."</td>
                <td>".($row['transmision'] === 'A' ? 'Automática' : ($row['transmision'] === 'M' ? 'Manual' : 'Desconocida'))."</td>
                <td>".htmlspecialchars($row['nombre'])."</td>
                <td>".($row['fechaStock'] ? $row['fechaStock']->format('Y-m-d') : 'N/A')."</td>
                <td class='acciones'>
                    <a href='editar.php?id=".$row['idauto']."'>Editar</a>
                    <a href='eliminar.php?id=".$row['idauto']."' onclick='return confirm(\"¿Estás seguro de eliminar este auto?\")'>Eliminar</a>
                </td>
            </tr>";
        }
